# web_routes.ps1 - shared HTTP route surface for the web server.
#
# Dot-sourced by BOTH the accept-loop process (web_server.ps1) and the slow-lane
# worker runspace, so a handler runs identically wherever it is dispatched.
# It must never open a listener or claim the PID file - that stays in web_server.ps1.
#
# Why a separate file instead of a table of [scriptblock] handlers: a scriptblock
# carries a reference to the SessionState it was created in, so passing one across a
# runspace boundary in PS 5.1 either throws or silently binds to the wrong scope. Only
# the HttpListenerContext (a plain .NET object) crosses the boundary here; the code
# itself is dot-sourced into each runspace independently.
#
# Requires: scripts_init.ps1 already dot-sourced (Write-Log, $msg, module functions,
# and this runspace's own database connection - ADR-0017), plus $ScriptPath and
# $ConfigFilePath set at script scope.

$websitePath = Join-Path $ScriptPath "website"

# Above this size a static file is streamed to the socket instead of being read into a
# byte array first. 1 MB keeps every page, script and icon on the cheap path while the
# toolbox zip under website\generated\ (a mapped .zip MIME type) never lands on the
# large object heap.
$script:StaticStreamThreshold = 1MB

# $PidFile is a web_server.ps1 parameter, so it is $null inside the slow-lane worker
# runspace. /api/restartwebserver embeds it in the detached killer script, which would
# then silently skip releasing data\webserver.pid. Resolve it from the canonical helper
# instead of depending on the caller's scope. The guard keeps web_server.ps1's own
# parameter authoritative when this file is dot-sourced there.
if (-not $PidFile) {
    if (Get-Command Get-WebServerPidPath -ErrorAction SilentlyContinue) {
        $PidFile = Get-WebServerPidPath
    } else {
        $PidFile = Join-Path $ScriptPath "data\webserver.pid"
    }
}

# Written by the persistent USB detection job in web_server.ps1, read by
# /api/usbdeviceinfo. That route is on the fast lane so it always runs in the accept
# process, but defining the path here keeps the read correct if it is ever reclassified.
if (-not $script:usbInfoResultFile) {
...
# ---------------------------------------------------------------------------
# Request lanes (ADR-0019)
#
# 'fast'  - pure database read, in-memory, or static file. Runs inline on the
#           accept loop. A route belongs here only if it provably cannot block.
# 'slow'  - touches ADB, the LAN, the internet, a child process or a human. The
#           accept loop hands the context to the worker runspace and returns to
#           GetContext() immediately, so one slow request can no longer freeze
#           every open page. Slow requests serialise among themselves, which is
#           what we want: two concurrent ADB scans contend on one transport.
#
# The default is 'fast', so a route added without thinking about it keeps the
# old behaviour rather than silently queueing behind a one-minute download.
# Deliberate exception: /proxy/mediamtx stays fast. It is already hardened to a
# 1500 ms timeout and sits on the video pages' hot path, where queueing it behind
# a binary download would be worse than the bounded stall it can cause.
# ---------------------------------------------------------------------------

$script:SlowRoutes = @{
    # Blocks on a human: Start-Process -Wait on a FolderBrowserDialog.
    '/api/browse-folder'                = $true
    # GitHub downloads - tens of seconds to a minute.
    '/api/ffmpeg-update'                = $true
    '/api/mediamtx-update'              = $true
    '/api/scrcpy-update'                = $true
    '/api/ffmpeg-check-update'          = $true
    '/api/mediamtx-check-update'        = $true
    '/api/scrcpy-check-update'          = $true
    # Whole-subnet sweep.
    '/api/kiosks/scan'                  = $true
    '/api/kiosks/networks'              = $true
    # Sleeps, process restarts, full-table WMI.
    '/api/restartmediamtx'              = $true
    '/api/config/save'                  = $true
    '/api/config/reset'                 = $true
    '/api/app-shutdown'                 = $true
    '/api/app-restart'                  = $true
    '/api/host-shutdown'                = $true
    '/api/restartwebserver'             = $true
    '/api/shutdown-all'                 = $true
    # Direct ADB round-trips.
    '/api/reboot'                       = $true
    '/api/shutdown'                     = $true
    '/api/headset-settings'             = $true
    '/api/headset-settings/apply'       = $true
    '/api/headset-firmware'             = $true
    '/api/headset-storage'              = $true
    '/api/installedapps'                = $true
    '/api/uninstallapp'                 = $true
    '/api/update-app'                   = $true
    '/api/launchapp'                    = $true
    '/api/foregroundapp'                = $true
    '/api/headset-connection-check'     = $true
    '/api/refresh-headset-apps-cache'   = $true
    '/api/installapk'                   = $true
    '/api/install-cancel'               = $true
    '/api/detectusbheadset'             = $true
    '/api/enablewifiadb'                = $true
    '/api/installadbwifiapk'            = $true
    '/api/connectwifi'                  = $true
    '/api/addheadset'                   = $true
    '/api/headsets/register-by-serial'  = $true
    '/api/appnames/clear'               = $true
    # scrcpy process control.
    '/api/stop-scrcpy'                  = $true
    '/api/start-scrcpy-all'             = $true
    '/api/stop-scrcpy-all'              = $true
    # Remote kiosk PCs over CDP/HTTP, and netsh.
    '/api/kiosks/push'                  = $true
    '/api/kiosks/kill-browser'          = $true
    '/api/kiosks/cdp-info'              = $true
    '/api/kiosks/power'                 = $true
    '/api/companion-info'               = $true
    '/api/companion-settings'           = $true
    '/api/wifi-detect'                  = $true
}

function Get-WebRouteLane {
    # Returns 'fast' or 'slow' for one request path. Method is not part of the key:
    # no path in this app is fast under one verb and slow under another.
    param([string]$Path)
    if ($Path -and $script:SlowRoutes.ContainsKey($Path)) { return 'slow' }
    return 'fast'
}

function Invoke-HttpRoute {
    # The whole route chain. Called by the accept loop for a fast route, or by the
    # slow-lane worker runspace for a slow one. Owns $response.Close() either way.
    param($Context)
    ...
    $context = $Context
            ...
            try {
                ...
                elseif ($target.SerialNumber) {
                    ...
                    if ($r.Ok) {
                        ...
                        $respBytes = [System.Text.Encoding]::UTF8.GetBytes((ConvertTo-Json @{ ok = $false; error = [string]$errText } -Compress))
                    }
                }
        ...
        if ($request.HttpMethod -eq 'GET' -and $request.Url.LocalPath -eq '/api/resolve-progress') {
            try {
                if (-not $script:resolveJob -and -not (Test-Path -LiteralPath $script:resolveProgFile)) {
                    ...
                    # No active job - only trust terminal statuses; stale running/queued becomes idle
                    ...
                    if ($json.tempId) {
                        ...
                        if (-not $overwrite) {
                            ...
                            if ($isInstalled) {
                                ...
                                return  # finally will close response
                            }
                        }
        ...
        if ($request.HttpMethod -eq 'POST' -and $request.Url.LocalPath -eq '/api/appnames/clear') {
            try {
                ...
                try {
                    $eb = [System.Text.Encoding]::UTF8.GetBytes((ConvertTo-Json @{ ok = $false; error = [string]$_ } -Compress))
                    $response.StatusCode = 500; $response.ContentType = 'application/json; charset=utf-8'
                    $response.ContentLength64 = $eb.Length; $response.OutputStream.Write($eb, 0, $eb.Length)
                } catch {}
            } finally { $response.Close() }
        ...
        if ($request.HttpMethod -eq 'POST' -and $request.Url.LocalPath -eq '/api/config/save') {
            try {
                ...
                try {
                    ...
                    $eb = [System.Text.Encoding]::UTF8.GetBytes((ConvertTo-Json @{ ok = $false; error = [string]$errMsg } -Compress))
        ...
        # API: GET /api/mediamtx-list-versions - lists all mediamtx version folders
        # present under sources\MediaMTX\ (old versions are never deleted by
        # /api/mediamtx-update, so more than one can coexist)
        if ($request.HttpMethod -eq 'GET' -and $request.Url.LocalPath -eq '/api/mediamtx-list-versions') {
            try {
                $versions = Get-MediaMtxInstalledVersions | ForEach-Object {
                    @{ version = $_.Version; folder = $_.RelativeFolder; active = $_.IsActive }
                }
                Send-JsonResponse -Response $response -Body @{ ok = $true; versions = @($versions) }
            } catch {
        ...
        if ($request.HttpMethod -eq 'POST' -and $request.Url.LocalPath -eq '/api/mediamtx-switch-version') {
            ...
            try {
                ...
                $sourcesFolder = Join-Path $ScriptPath "sources"
                $mediamtxRoot  = Join-Path $sourcesFolder "MediaMTX"
                $requestedFolder = Join-Path $sourcesFolder ([string]$body.folder)
                $resolved = Test-RequestPath -Path $requestedFolder -Root $mediamtxRoot
                if (-not $resolved -or -not (Test-Path -LiteralPath (Join-Path $resolved "mediamtx.exe"))) {
                    Send-JsonResponse -Response $response -StatusCode 400 -Body @{ ok = $false; error = "Invalid or missing mediamtx version folder." }
                    $response.Close()
                    return
                }
                $relFolder = Join-Path "MediaMTX" (Split-Path -Path $resolved -Leaf)
                ...
                $switchError = $null
                try {
                    Set-MediaMtxFolderConfig -RelativeFolder $relFolder
                } catch {
                    Write-Log ("mediamtx-switch-version: Set-MediaMtxFolderConfig failed: " + $_.Exception.Message) -Level WARNING
                    $switchOk = $false
                    $switchError = $_.Exception.Message
                }
        ...
        # API: GET /api/scrcpy-list-versions - lists all scrcpy version folders
        # present under sources\scrcpy\ (old versions are never deleted by
        # /api/scrcpy-update, so more than one can coexist)
        if ($request.HttpMethod -eq 'GET' -and $request.Url.LocalPath -eq '/api/scrcpy-list-versions') {
            try {
                $versions = Get-ScrcpyInstalledVersions | ForEach-Object {
                    @{ version = $_.Version; folder = $_.RelativeFolder; active = $_.IsActive }
                }
                Send-JsonResponse -Response $response -Body @{ ok = $true; versions = @($versions) }
            } catch {
        ...
        if ($request.HttpMethod -eq 'POST' -and $request.Url.LocalPath -eq '/api/scrcpy-switch-version') {
            ...
            try {
                ...
                $sourcesFolder = Join-Path $ScriptPath "sources"
                $scrcpyRoot    = Join-Path $sourcesFolder "scrcpy"
                $requestedFolder = Join-Path $sourcesFolder ([string]$body.folder)
                $resolved = Test-RequestPath -Path $requestedFolder -Root $scrcpyRoot
                if (-not $resolved -or -not (Test-Path -LiteralPath (Join-Path $resolved "scrcpy.exe"))) {
                    Send-JsonResponse -Response $response -StatusCode 400 -Body @{ ok = $false; error = "Invalid or missing scrcpy version folder." }
                    $response.Close()
                    return
                }
                $relFolder = Join-Path "scrcpy" (Split-Path -Path $resolved -Leaf)
                ...
                $switchError = $null
                try {
                    Set-ScrcpyAdbFolderConfig -RelativeFolder $relFolder
                } catch {
                    Write-Log ("scrcpy-switch-version: Set-ScrcpyAdbFolderConfig failed: " + $_.Exception.Message) -Level WARNING
                    $switchOk = $false
                    $switchError = $_.Exception.Message
                }
        ...
        if ($request.HttpMethod -eq 'GET' -and $request.Url.LocalPath -eq '/api/timer') {
            try {
                ...
                try {
                    $eb = [System.Text.Encoding]::UTF8.GetBytes((ConvertTo-Json @{ ok = $false; error = [string]$_.Exception.Message } -Compress))
                    $response.StatusCode = 400; $response.ContentType = 'application/json; charset=utf-8'
                    $response.ContentLength64 = $eb.Length; $response.OutputStream.Write($eb, 0, $eb.Length)
                } catch {}
            } finally { $response.Close() }
        ...
        try {
            ...
            if ($urlPath.StartsWith("data" + [System.IO.Path]::DirectorySeparatorChar)) {
                ...
                # Path-traversal guard. Test-RequestPath appends the directory separator
                # before comparing; the bare StartsWith this replaces did not, so a path
                # resolving to "<parent>\websiteX\..." passed the check purely because
                # "websiteX" starts with "website". ADR-0012 already names this helper as
                # the sanctioned guard for file-serving routes.
                $resolvedFile = Test-RequestPath -Path (Join-Path $websitePath $urlPath) -Root $websitePath
                if (-not $resolvedFile) {
                    $response.StatusCode = 403
                    $response.Close()
                    return
                ...
                if (-not [System.IO.File]::Exists($resolvedFile)) {
                    $generatedBase = Join-Path $websitePath "generated"
                    $fallback      = Test-RequestPath -Path (Join-Path $generatedBase $urlPath) -Root $generatedBase
                    if ($fallback -and [System.IO.File]::Exists($fallback)) {
            ...
            if ([System.IO.File]::Exists($resolvedFile)) {
                ...
                $fi   = New-Object System.IO.FileInfo $resolvedFile

                # HTTP dates carry no sub-second component, so the mtime is truncated to
                # whole seconds before it is compared or formatted - otherwise the file
                # always looks newer than the date we ourselves sent and every conditional
                # request would be a miss.
                $mtimeUtc = $fi.LastWriteTimeUtc
                $mtimeUtc = $mtimeUtc.AddTicks(-($mtimeUtc.Ticks % [timespan]::TicksPerSecond))
                $lastMod  = $mtimeUtc.ToString('R')

                # Conditional request. This route used to SET Last-Modified but never READ
                # If-Modified-Since, so every asset on every page load was re-read from disk
                # and re-sent in full. That is what made a whole-file ReadAllBytes expensive.
                $ims = $request.Headers['If-Modified-Since']
                if ($ims) {
                    $imsDate = [datetime]::MinValue
                    $styles  = [System.Globalization.DateTimeStyles]::AdjustToUniversal -bor `
                               [System.Globalization.DateTimeStyles]::AssumeUniversal
                    if ([datetime]::TryParse($ims, [System.Globalization.CultureInfo]::InvariantCulture, $styles, [ref]$imsDate)) {
                        if ($mtimeUtc -le $imsDate) {
                ...
                # body and the 304 above is what makes revalidating cheap. Deliberately not
                # max-age - a generated overlay must never be served stale from cache.
                $response.Headers.Add('Cache-Control', 'no-cache')

                if ($fi.Length -gt $script:StaticStreamThreshold) {
                    # Stream rather than materialise the whole file. website\generated\
                    # holds the headset-toolbox zip and .zip is a mapped MIME type, so
                    # ReadAllBytes put the entire archive on the large object heap for
                    # every download, twice over while the response buffer coexisted.
                    $fs = [System.IO.File]::OpenRead($resolvedFile)
                    try { $fs.CopyTo($response.OutputStream, 81920) } finally { $fs.Dispose() }
                } else {
